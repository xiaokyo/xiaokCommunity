...
export default Form.create({ name: 'register' })(props => {
	...
	const handleSubmit = e => {
		...
		props.form.validateFields((err, values) => {
			if (!err) {
				...
				const toPath = props.location.state ? props.location.state.from.pathname : '/login';
	...
	const regUser = async (username, password) => {
		const args = `addUser($username:String!,$password:String!){
      addUser(username:$username,password:$password){
		...
		if (!res.addUser.success) throw new Error('注册失败');
